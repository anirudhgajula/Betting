...
describe("Testing Contract with 20 Persons - All Same Bet Size", function() {
    async function deployFixture() {
        ...
        const betting = await Betting.connect(owner).deploy(token.address, 20);
...
describe("Testing Contract with 5 Persons - All Diff Bet Size", function() {
    async function deployFixture() {
        ...
        const Betting = await ethers.getContractFactory("Betting");
        // ensure 1 million betSize in betting contract with 20 betters
        const betting = await Betting.connect(owner).deploy(token.address, 5);
        return { owner, betters, token, betting };
    };
    ...
    describe("Betting Process with Different Bet Sizes", function() {
        it("Should end the process successfully", async function() {
            ...
            await token.approve(betting.address, ethers.utils.parseUnits('15', '24'));
            await betting.disburseFunds(1);
            for (let i = 0; i < 5; i++) {
                if (i % 2 == 1) {
                    expect(await token.balanceOf(betters[i].address)).to.equal(ethers.utils.parseUnits((5 * (i + 1) / 2).toString(), '24'));
                }
                else {
                    expect(await token.balanceOf(betters[i].address)).to.equal(0);
                }
